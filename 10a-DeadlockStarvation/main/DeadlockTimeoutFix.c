 ...
 * This is fixed by timing out if a task can't take a mutex.
*/
#include <stdio.h>
...
void deadlock_task_A(void* param) {
    while (true) {
        if (xSemaphoreTake(mutex1, MUTEX_TAKE_TIMEOUT) == pdTRUE) {
            ESP_LOGI(TAG, "Task A took mutex 1");
            vTaskDelay(pdMS_TO_TICKS(10));

            if (xSemaphoreTake(mutex2, MUTEX_TAKE_TIMEOUT) == pdTRUE) {
                ESP_LOGI(TAG, "Task A took mutex 2");

                ESP_LOGI(TAG, "Task A doing work");
                vTaskDelay(pdMS_TO_TICKS(500));

                // Must give a mutex only if it's actually taken
                xSemaphoreGive(mutex2);
            }
            else {
                ESP_LOGI(TAG, "Task A timed out waiting for mutex 2");
            }

            xSemaphoreGive(mutex1);
        }
        else {
            ESP_LOGI(TAG, "Task A timed out waiting for mutex 1");
        }
    ...
    while (true) {
        if (xSemaphoreTake(mutex2, MUTEX_TAKE_TIMEOUT) == pdTRUE) {
            ...
            if (xSemaphoreTake(mutex1, MUTEX_TAKE_TIMEOUT) == pdTRUE) {
                ...
                ESP_LOGI(TAG, "Task B timed out waiting for mutex 1");
            }
            
            xSemaphoreGive(mutex2);
        }
        else {
            ESP_LOGI(TAG, "Task B timed out waiting for mutex 2");
        }
...
void app_main(void) {
    ...
    ESP_LOGI(TAG, "-----Deadlock Timeout Fix Example-----");
